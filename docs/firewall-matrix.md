...
- **IoT DNS:** `homelab.router.enableBlocky` is **true** on janus. On → DHCP DNS is Blocky; no IoT `domain-search`; prerouting **DNAT** `:53`/`:853` to `10.10.30.21` (port preserved, no SNAT so Blocky sees real client IPs — TrueNAS default gateway must be `10.10.30.1`); INPUT drop and WAN `:53`/`:853` drop (`oifname WAN`) are fallbacks. Off (bring-up only) → DHCP DNS is Unbound on `10.10.40.1`, INPUT `:53` from IoT is allowed, `domain-search lab.zdk.no` is pushed. **IPv6:** same DNAT in `table ip6 nat` when `enableIpv6` is on **and** `blockyIpv6` is set to Blocky’s GUA (typically `<servers-/64>::21`). If v6 is on but `blockyIpv6` is null, IPv6 DNS to WAN is dropped so clients use IPv4 intercept. No IPv6 masquerade (native /64s). DoH on `:443` is not intercepted (would break HTTPS).
...
- Rule intent lives in `router/modules/firewall.nix`.
